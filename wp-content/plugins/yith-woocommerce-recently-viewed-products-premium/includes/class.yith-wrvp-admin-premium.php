...
if ( ! class_exists( 'YITH_WRVP_Admin_Premium' ) ) {
	...
	class YITH_WRVP_Admin_Premium extends YITH_WRVP_Admin {
		...
		public function enqueue_scripts() {
			wp_register_script( 'yith-wrvp-admin', YITH_WRVP_ASSETS_URL . '/js/yith-wrvp-admin.js', array( 'jquery' ), false, true );
		...
		public function shortcode_tab() {
			...
			$premium_tab_template = YITH_WRVP_TEMPLATE_PATH . '/admin/shortcode-tab.php';
		...
		public function custom_image_size( $value ){
			...
			<td class="forminp yith_image_size_settings"
				...
				<label><input name="<?php echo esc_attr( $value['id'] ); ?>[crop]" id="<?php echo esc_attr( $value['id'] ); ?>-crop" type="checkbox" value="1" <?php checked( 1, $crop ); ?> /> <?php _e( 'Do you want to hard crop the image?', 'yith-woocommerce-recently-viewed-products' ); ?></label>
